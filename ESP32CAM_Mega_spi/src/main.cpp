...
#define I2C_SDA 16  //to sensor white
#define I2C_SCL 15  //to sensor green
...
static esp_err_t cmd_handler(httpd_req_t *req){
  ...
  if(!strcmp(variable, "forward")) {
    Serial.println("f");
    char c;
    digitalWrite(ESP32_CAM_SS, LOW); // enable Slave Select
    for (const char * p = "f\r" ; c = *p; p++) {
      SPI.transfer(c);
    }
    digitalWrite(ESP32_CAM_SS, HIGH); // disable Slave Select
  }
  else if(!strcmp(variable, "left")) {
    Serial.println("l");
    char c;
    digitalWrite(ESP32_CAM_SS, LOW);
    for (const char * p = "l\r" ; c = *p; p++) {
      SPI.transfer(c);
    }
    digitalWrite(ESP32_CAM_SS, HIGH);
  }
